 ...
 */
    ...
    public function __construct( $plugin_file, $args = array() ) {
        ...
        $this->enable_self_cron = true == $this->disabled_wp_cron ? true : false;
    ...
    public function init() {
        ...
        /**
         * Self-heal the schedule. A file-only plugin update does NOT fire
         * register_activation_hook(), so already-opted-in sites whose cron was
         * never scheduled (or was lost) would otherwise never report again. Since
         * init() runs on every request, re-ensure the event whenever tracking is
         * allowed. schedule_tracking() guards with wp_next_scheduled(), so this is
         * idempotent and respects DISABLE_WP_CRON.
         */
    ...
    private function redirect_to() {
        $request_payload = isset( $_SERVER[ 'REQUEST_URI' ] ) ? esc_url_raw( wp_unslash( $_SERVER[ 'REQUEST_URI' ] ) ) : '';
        $request_uri     = wp_parse_url( $request_payload, PHP_URL_PATH );
        $query_string    = wp_parse_url( $request_payload, PHP_URL_QUERY );
    ...
    public function get_data() {
        ...
        $body[ 'server' ]           = isset( $_SERVER[ 'SERVER_SOFTWARE' ] ) ? $_SERVER[ 'SERVER_SOFTWARE' ] : ''; //phpcs:ignore
        ...
        if ( empty( $plugin ) ) {
            $body[ 'message' ] .= __( 'We can\'t detect any plugin information. This is most probably because you have not included the code in the plugin main file.', 'betterdocs' );
        ...
        /**
         * Extension point for product-usage analytics.
         *
         * Collectors (Free / Pro / Chatbot) hook this to inject their `bd_*`
         * metric groups into `$body['optional_data']` (a nested array). See
         * `WPDeveloper\BetterDocs\Insights\Collector` and `docs/insights-tracking.md`.
         *
         * @param array    $body The tracking payload.
         * @param Insights $this The Insights instance.
         */
        $body = apply_filters( 'betterdocs_insights_data', $body, $this );

        /**
         * wpinsight persists only the fixed base fields plus a single
         * `optional_data` field, which it expects as a JSON-encoded string.
         * Collectors assemble it as a nested array; here we JSON-encode each
         * first-layer group (bd_features, bd_counts, …) into its own string
         * value, then JSON-encode the whole `optional_data` object.
         */
        if ( ! empty( $body[ 'optional_data' ] ) && is_array( $body[ 'optional_data' ] ) ) {
            foreach ( $body[ 'optional_data' ] as $group => $value ) {
                if ( is_array( $value ) ) {
                    $body[ 'optional_data' ][ $group ] = wp_json_encode( $value );
                }
            }
            $body[ 'optional_data' ] = wp_json_encode( $body[ 'optional_data' ] );
        }
    ...
    public function send_data( $body ) {
        ...
        // Re-register when the site has no recorded origin URL yet, or when the
        // current URL no longer matches it (domain migration). Without the URL-change
        // arm (added in tracker 3.0.3), a moved site keeps reporting under its old
        // site ID with stale identity data. Mirrors EA Lite's 3.0.3 send_data().
        if ( ( false === $original_site_url || $original_site_url != $site_url ) && version_compare( $body[ 'wpins_version' ], '3.0.1', '>=' ) ) {
            $site_id = false;
        }
        /**
    ...
    protected function remote_post( $data = array(), $args = array() ) {
        ...
        if ( is_wp_error( $request ) || ( isset( $request[ 'response' ], $request[ 'response' ][ 'code' ] ) && 200 != $request[ 'response' ][ 'code' ] ) ) {
    ...
    public function notice() {
		...
		<?php
            ...
            'consent_button_text' => __( 'What we collect.', 'betterdocs' ),
            'yes' => __( 'Sure, I\'d like to help', 'betterdocs' ),
            'no' => __( 'No Thanks.', 'betterdocs' )
                    );
            ...
            if ( ! wp_verify_nonce( $nonce, '_wpnonce_optin_' . $this->plugin_name ) ) {
                return;
            }
         ...
         */
            ...
            $links[ 'deactivate' ] = $deactivation_link;
                ...
                public function deactivation_reasons() {
                    $form              = array();
                    $form[ 'heading' ] = __( 'We Value Your Feedback', 'betterdocs' );
                    $form[ 'body' ]    = __( "Could you please tell us why you're deactivating our plugin? Your insights will help us to improve.", 'betterdocs' );

                    $form[ 'options' ] = array(
            __( 'I no longer need the plugin', 'betterdocs' ),
            array(
                'label' => __( 'I found a better plugin', 'betterdocs' ),
                'extra_field' => __( 'Please share which plugin', 'betterdocs' )
            ),
            __( "I couldn't get the plugin to work", 'betterdocs' ),
            __( 'It\'s a temporary deactivation', 'betterdocs' ),
            array(
                'label' => __( 'Other', 'betterdocs' ),
                'extra_field' => __( 'Please share the reason', 'betterdocs' ),
                'type' => 'textarea'
            )
                    );
                    // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP Insights (appsero) library hook; name is defined by the upstream tracker library.
            ...
            $html .= '</ul></div><!-- .wpinsights-' . $class_plugin_name . '-goodbye-options -->';
                    ...
                    $html .= '<p class="deactivating-spinner"><span class="spinner"></span> ' . __( 'Submitting form', 'betterdocs' ) . '</p>';
                ...
                .wpinsights-goodbye-form .betterdocs__modal-close-btn {
                    position: absolute;
                    top: 0;
                    right: -55px;
                    width: 40px;
                ...
                .wpinsights-goodbye-form .betterdocs__modal-close-btn svg {
                    width: 20px;
                }
                ...
                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-head > span {
                    display: inline-flex;
                }
                ...
                .wpinsights-goodbye-form-wrapper-betterdocs .widefat td ul {
                    font-size: 14px;
                    margin: 0;
                    color: #475467;
                    line-height: 1.4em;
                }
                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-body #wpinsights-goodbye-options ul > li:not(:last-child) {
                    margin-bottom: 16px;
                }
                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-body #wpinsights-goodbye-options ul > li:last-child {
                    margin-bottom: 0;
                }
                .wpinsights-goodbye-form-wrapper-betterdocs input[type=radio] {
                    border-radius: 50%;
                    margin-right: .25rem;
                ...
                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-body #wpinsights-goodbye-options ul > li > div > input, .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-body #wpinsights-goodbye-options ul > li > div > textarea {
                    padding: 10px 16px;
                    border-radius: 8px;
                    width: 100%;
                ...
                .wpinsights-goodbye-form-wrapper-betterdocs input[type=radio]:checked::before {
                    content: "";
                    border-radius: 50%;
                    width: .88rem;
                    height: .88rem;
                    ...
                    border: 1px solid #e0fff6;
                    line-height: 1.14285714;
                    outline: 0;
                }
                .wpinsights-goodbye-form-wrapper-betterdocs input[type=radio]:focus {
                    border-color: #D0D5DD;
                    box-shadow: none;
                    outline: 0;
                }
                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-body #wpinsights-goodbye-options ul > li > div > input::placeholder,
                    .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-body #wpinsights-goodbye-options ul > li > div > textarea::placeholder{
                ...
                .wpinsights-goodbye-form-wrapper-betterdocs input:focus,
                .wpinsights-goodbye-form-wrapper-betterdocs textarea:focus {
                ...
                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-footer .wpinsights-submit-btn:hover {
                    background: #00b884;
                }

                .wpinsights-goodbye-form-wrapper-betterdocs .wpinsights-goodbye-form-footer .wpsp-put-deactivate-btn {
                    font-size: 14px;
                    font-weight: 500;
                    color: #344054;
                    margin-left: 10px;
            ...
            ';
                    ...
                    echo $styles; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- This is styles, which is why escaping is not needed
				...
				$("#wpinsights-goodbye-link-<?php echo esc_attr( $class_plugin_name ); ?>").on("click",function(){
					...
					$('#wpinsights-submit-form-<?php echo esc_attr( $class_plugin_name ); ?>').on('click', function(e){
						...
						var data = {
							...
							'details': details,
							'security': "<?php echo wp_create_nonce( 'wpins_deactivation_nonce' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped       ?>",
							'dataType': "json"
						}
					...
					$('.wpinsights-goodbye-form-bg, #wpinsights-goodbye-form > .betterdocs__modal-close-btn').on('click',function(){
