...
/***/ "./react-src/admin/scss/setup-wizard.scss":
/*!************************************************!*\
  !*** ./react-src/admin/scss/setup-wizard.scss ***!
  \************************************************/
...
(() => {
/*!*****************************************!*\
  !*** ./react-src/admin/setup-wizard.js ***!
  \*****************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _scss_setup_wizard_scss__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./scss/setup-wizard.scss */ "./react-src/admin/scss/setup-wizard.scss");
...
jQuery(document).ready(function ($) {
    ...
    function betterdocsQswNextPrev(n) {
      ...
      if (currentTab < x.length) {
        ...
        swal({
          ...
          let pageReplace = location.search.replace("betterdocs-setup", "betterdocs-settings");
          location = location.origin + location.pathname + location.search.replace("betterdocs-setup", "betterdocs-settings") + '&saved=true';
        });
  ...
  function betterdocsQswOptionSubmit() {
    ...
    var builtin_doc_page = isChecked('.betterdocs-setup-wizard input[name="builtin_doc_page"]');
    var enable_disable = isChecked('.betterdocs-setup-wizard input[name="enable_disable"]');
    var docs_slug = $('.betterdocs-setup-wizard input[name="docs_slug"]').val();
    var activatePlugin = isChecked(".betterdocs-setup-wizard .activate_plugin");
    var activate_plugin = "";
    if (activatePlugin) {
      activate_plugin = $(".betterdocs-setup-wizard .activate_plugin").val();
    }
    var data = {
  ...
  function betterdocsQswOptinSubmit() {
    var quickOptin = $("#betterdocs_user_email_address");
    if (quickOptin.is(":checked")) {
      var ajaxnonce = $('.betterdocs-setup-wizard input[name="betterdocsqswnonce"]').val();
      var data = {
        nonce: ajaxnonce,
